...
# Adapted from https://www.tensorflow.org/tutorials/generative/adversarial_fgsm
...
def get_constraint_accuracy_random(model, images, labels_unused, epsilon, delta):
    ...
    labels_perturbed_images = model.predict(images_perturbed)
    for i in range(labels_original_images.shape[0]):
        if np.linalg.norm(labels_original_images[i] - labels_perturbed_images[i]) < delta:
            counter+=1
    return counter/images.shape[0]
